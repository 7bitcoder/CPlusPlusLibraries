...
{
    auto time = dev::Miliseconds(2);

    EXPECT_EQ(time, (Time{0, 0, 0, 0, 2, 0}));
    EXPECT_EQ(time.totalMiliseconds(), 2.0);
}

TEST_F(TimeTest, FromMicrosecondsLongLongTest)
{
    auto time = dev::Microseconds(2);
...
{
    Time time = 2_ds + 2_h + 1_min + 1_s + 1_ms + 2_us;
